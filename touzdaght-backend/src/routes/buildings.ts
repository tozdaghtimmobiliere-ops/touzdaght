...
router.post('/', requireAuth, requireSuperAdmin, async (req, res) => {
  try {
    const { projectId, name, floorsCount, apartmentsPerFloor, totalSurface, floorLabels, tableauSection } = req.body
    ...
    const building = await prisma.building.create({
      data: {
        ...
        floorLabels: floorLabels ? JSON.stringify(floorLabels) : null,
        tableauSection: tableauSection || null,
      },
    })

    // Auto-generate apartments
    const apartments = []
    for (let floor = 0; floor < parseInt(floorsCount); floor++) {
      for (let unit = 1; unit <= parseInt(apartmentsPerFloor); unit++) {
        apartments.push({
...
router.put('/:id', requireAuth, requireSuperAdmin, async (req, res) => {
  try {
    const { id } = req.params
    const { name, floorsCount, apartmentsPerFloor, totalSurface, floorLabels, tableauSection } = req.body
    ...
    const building = await prisma.building.update({
      ...
      data: {
        ...
        floorLabels: floorLabels !== undefined ? (floorLabels ? JSON.stringify(floorLabels) : null) : undefined,
        tableauSection: tableauSection !== undefined ? tableauSection : undefined,
      },
    })
